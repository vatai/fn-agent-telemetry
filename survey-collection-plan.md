...
3. **Done.** Snapshot the session transcript. Assistant output, token usage and cost are all absent from the nine hook payloads but present in the transcript at `transcript_path`, so copy it to `<AGENT_TELEMETRY_DIR>/transcripts/<session_id>.jsonl` on `Stop` and `SessionEnd`, with the event log moving alongside it to `<AGENT_TELEMETRY_DIR>/agent-telemetry.jsonl`, plus a `/snapshot` command for manual runs. Note the Specification describes only the `AGENT_TELEMETRY_LOG` JSONL and is silent on transcript copies as a second artifact.
