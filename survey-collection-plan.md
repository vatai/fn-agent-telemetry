...
- Append plaintext JSONL telemetry indefinitely under the directory configured by `AGENT_TELEMETRY_DIR`, one file per session.
...
3. **Done.** Snapshot the session transcript. Assistant output, token usage and cost are all absent from the nine hook payloads but present in the transcript at `transcript_path`, so copy it to `<AGENT_TELEMETRY_DIR>/transcripts/<session_id>.jsonl` on `Stop` and `SessionEnd`, with the event log alongside it at `<AGENT_TELEMETRY_DIR>/events/<session_id>.jsonl`, plus a `/snapshot` command for manual runs.
